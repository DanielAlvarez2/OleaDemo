...
app.get('/wineFormatLayout', (request, response) =>{
...
app.get('/nonAlcoholic', (request, response) =>{
    db.collection('Specials').find().sort({price:1}).toArray()
    .then(data => {
        response.render('nonAlcoholic.ejs', {info: data})
    })
})
...
app.post('/addBeer', async(request,response)=>{
    await db.collection('Specials').insertOne({
        category: `${request.body.category}`,
        type: `${request.body.type}`,
        name: `${request.body.name}`,
...
app.post('/addSherry', async(request,response)=>{
    await db.collection('Specials').insertOne({
        category: `${request.body.category}`,
        grapes: `${request.body.grapes}`,
        name: `${request.body.name}`,
...
app.post('/addSparkling', async(request,response)=>{
    await db.collection('Specials').insertOne({
        category: `${request.body.category}`,
        type: `${request.body.type}`,
        grapes: `${request.body.grapes}`,
...
app.post('/editNonAlcoholic', async(request,response)=>{
    await db.collection('Specials').updateOne({
            _id: new ObjectId(`${request.body._id}`)
    },{
        $set:{
...
app.post('/editSherry', async(request,response)=>{
    await db.collection('Specials').updateOne({
            _id: new ObjectId(`${request.body._id}`)
    },{
        $set:{
